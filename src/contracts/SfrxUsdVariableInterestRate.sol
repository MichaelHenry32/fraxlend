...
// ===============================================================
// ================= SfrxUsdVariableInterestRate =================
// ===============================================================

// The goal of this smart contract is to allow for sfrxUSD markets to have identical slopes to frxUSD markets.
// So, all constructor variables remain in terms of frxUSD, as do all outputs besides _newRatePerSec
// To do so, the current strategy is to grab the frxUSDRPS and subtract sfrxUSD's interent frxUSDRPS  before converting the rate to sfrxUSD.
// Still very much a WIP

// Heavily forked from the Frax Finance team VariableInterestRate.sol contract
// But the original work of MichaelHenry32
...
contract VariableInterestRate is IRateCalculatorV2 {
  ...
  function getNewFrxUsdRate(
    uint256 _deltaTime,
    uint256 _utilization,
    uint64 _oldFullUtilizationInterest
  ) internal view returns (uint64 _newRatePerSec, uint64 _newFullUtilizationInterest) {
    ...
    if (_utilization < VERTEX_UTILIZATION) {
      ...
      // 18 decimals
      _newRatePerSec = uint64(ZERO_UTIL_RATE + (_utilization * (_vertexInterest - ZERO_UTIL_RATE)) / VERTEX_UTILIZATION);
    } else {
      // For readability, the following formula is equivalent to:
  ...
  /// @param _utilization The utilization %, given with 5 decimals of precision
  /// @param _oldFullUtilizationInterest The interest value when utilization is 100%, given with 18 decimals of precision
  /// @return _newRatePerSec The new interest rate, 18 decimals of precision. In terms of sfrxUSD
  /// @return _newFullUtilizationInterest The new max interest rate, 18 decimals of precision. In terms of frxUSD.
  function getNewRate(
    uint256 _deltaTime,
    uint256 _utilization,
    uint64 _oldFullUtilizationInterest
